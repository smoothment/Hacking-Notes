...
Active Directory (AD) is used by approximately 90% of the Global Fortune 1000 companies. If an organisation's estate uses Microsoft Windows, you are almost guaranteed to findÂ AD. MicrosoftÂ ADÂ is the dominant suite used to manage Windows domain networks. However, sinceÂ ADÂ is used for Identity and Access Management of the entire estate, it holds the keys to the kingdom, making it a very likely target for attackers.

For a more in-depth understanding ofÂ ADÂ and how it works,Â [please complete this room onÂ ADÂ basics first.](https://tryhackme.com/jr/winadbasics)

Breaching Active Directory  

Before we can exploitÂ ADÂ misconfigurations for privilege escalation, lateral movement, and goal execution, you need initial access first. You need to acquire an initial set of validÂ ADÂ credentials. Due to the number ofÂ ADÂ services and features, the attack surface for gaining an initial set ofÂ ADÂ credentials is usually significant. In this room, we will discuss several avenues, but this is by no means an exhaustive list.

When looking for that first set of credentials, we don't focus on the permissions associated with the account; thus, even a low-privileged account would be sufficient. We are just looking for a way to authenticate toÂ AD, allowing us to do further enumeration onÂ ADÂ itself.

## Learning Objectives  

In this network, we will cover several methods that can be used to breachÂ AD. This is by no means a complete list as new methods and techniques are discovered every day. However, we willÂ  cover the following techniques to recoverÂ ADÂ credentials in this network:

- NTLMÂ Authenticated Services
...
You should also take the time to make note of your VPN IP. UsingÂ `ifconfig`Â orÂ `ip a`, make note of the IP of theÂ **breachad**Â network adapter. This is your IP and the associated interface that you should use when performing the attacks in the tasks.  

**Other Hosts**  

If you are going to use your own attack machine, an OpenVPN configuration file will have been generated for you once you join the room. Go to yourÂ [access](https://tryhackme.com/access)Â page. Select 'BreachingAD' from theÂ VPNÂ servers (under the network tab) and download your configuration file.
...
Use an OpenVPN client to connect. This example is shown on aÂ LinuxÂ machine; similar guides to connect using Windows or macOSÂ can be found at yourÂ [access](https://tryhackme.com/access)Â page.
...
**Note:**Â You still have to configureÂ DNSÂ similar to what was shown above. It is important to note that although not used, theÂ DCÂ does logÂ DNSÂ requests. If you are using your own machine, these logs may include the hostname of your device. For example, if you run theÂ VPNÂ on your kali machine with the hostname of kali, this will be logged.
...
- You can hardcode DNS entries into yourÂ `/etc/hosts`Â file. While this may work well, it is infeasible when you will be testing networks that have more than 10000 hosts.
- You can spend the time required to debug theÂ DNSÂ issue to get it working. While this may be harder, in the long run, it will yield you better results.

Whenever one of the tasks within this room is not working for you, your first thought should be:Â _Is myÂ DNSÂ working?_Â  From experience, I, the creator of this network, can tell you that I've wasted countless hours on assessments wondering why my tooling is not working, only to realise that myÂ DNSÂ has changed.  

Whenever you think that yourÂ DNSÂ configuration might not be working as it should, follow these steps to do some debugging:
...
2. RunÂ `ping <THM DC IP>`Â - This will verify that the network is active. If you do not get a response from the ping, it means that the network is not currently active. If your network says that it is running after you have refreshed the room page and you still get no ping response, contactÂ THMÂ support but simply waiting for the network timer to run out before starting the network again will fix the issue.
3. RunÂ `nslookup za.tryhackme.com <THM DC IP>`Â - This will verify that the DNS server within the network is active, as the domain controller has this functional role. If the ping command worked but this does not, time to contact support since there is something wrong. It is also suggested to hit the network reset button.
4. Finally, runÂ `nslookup tryhackme.com`Â - If you now get a different response than the one in step three, it means there is something wrong with yourÂ DNSÂ configuration. Go back to the configuration steps at the start of the task and follow them again. A common issue seen on Kali is that theÂ DNSÂ entry is placed as the second one in yourÂ `/etc/resolv.conf`Â file. By making it the first entry, it will resolve the issue.

TheseÂ ADÂ networks are rated medium, which means if you just joinedÂ THM, this is probably not where you should start your learning journey.Â ADÂ is massive and you will need to apply the mindset ofÂ _figuring stuff_Â _out_Â if you want to make a success of testing it. However, if all of the above still fails, please be as descriptive as possible on what you are trying to do when you contact support, to allow them to help you as efficiently as possible.
...
Two popular methods for gaining access to that first set ofÂ ADÂ credentials is Open Source Intelligence (OSINT) andÂ Phishing. We will only briefly mention the two methods here, as they are already covered more in-depth in other rooms.  

**OSINT**

OSINTÂ is used to discover information that has been publicly disclosed. In terms ofÂ ADÂ credentials, this can happen for several reasons, such as:

- Users who ask questions on public forums such asÂ [Stack Overflow](https://stackoverflow.com/)Â but disclose sensitive information such as their credentials in the question.
- Developers that upload scripts to services such asÂ [Github](https://github.com/)Â with credentials hardcoded.
- Credentials being disclosed in past breaches since employees used their work accounts to sign up for other external websites. Websites such asÂ [HaveIBeenPwned](https://haveibeenpwned.com/)Â andÂ [DeHashed](https://www.dehashed.com/)Â provide excellent platforms to determine if someone's information, such as work email, was ever involved in a publicly known data breach.  
...
By usingÂ OSINTÂ techniques, it may be possible to recover publicly disclosed credentials. If we are lucky enough to find credentials, we will still need to find a way to test whether they are valid or not sinceÂ OSINTÂ information can be outdated. In Task 3, we will talk aboutÂ NTLMÂ Authenticated Services, which may provide an excellent avenue to test credentials to see if they are still valid.

A detailed room on Red TeamÂ OSINTÂ can be foundÂ [here.](https://tryhackme.com/jr/redteamrecon)
...
PhishingÂ is another excellent method to breachÂ AD.Â PhishingÂ usually entices users to either provide their credentials on a malicious web page or ask them to run a specific application that would install a Remote Access Trojan (RAT) in the background. This is a prevalent method since the RAT would execute in the user's context, immediately allowing you to impersonate that user'sÂ ADÂ account. This is whyÂ phishingÂ is such a big topic for both Red and Blue teams.

A detailed room onÂ phishingÂ can be foundÂ [here.](https://tryhackme.com/module/phishing)

![Pasted image 20250526123341.png](../../../IMAGES/Pasted%20image%2020250526123341.png)
...
New Technology LAN Manager (NTLM) is the suite of security protocols used to authenticate users' identities inÂ AD.Â NTLMÂ can be used for authentication by using a challenge-response-based scheme called NetNTLM. This authentication mechanism is heavily used by the services on a network. However, services that use NetNTLM can also be exposed to the internet. The following are some of the popular examples:

- Internally-hosted Exchange (Mail) servers that expose an Outlook Web App (OWA) login portal.
- Remote Desktop Protocol (RDP) service of a server being exposed to the internet.
- ExposedÂ VPNÂ endpoints that were integrated withÂ AD.
- Web applications that are internet-facing and make use of NetNTLM.  
    

NetNTLM, also often referred to as Windows Authentication or justÂ NTLMÂ Authentication, allows the application to play the role of a middle man between the client andÂ AD. All authentication material is forwarded to a Domain Controller in the form of a challenge, and if completed successfully, the application will authenticate the user.

This means that the application is authenticating on behalf of the user and not authenticating the user directly on the application itself. This prevents the application from storingÂ ADÂ credentials, which should only be stored on a Domain Controller. This process is shown in the diagram below:
...
**Note:**Â _Firefox's Windows Authentication plugin is incredibly prone to failure. If you want to test credentials manually, Chrome is recommended._

We could use tools such asÂ [Hydra](https://github.com/vanhauser-thc/thc-hydra)Â to assist with the password spraying attack. However, it is often better to script up these types of attacks yourself, which allows you more control over the process. A base python script has been provided in the task files that can be used for the password spraying attack. The following function is the main component of the script:
...
This function takes our suggested password and the URL that we are targeting as input and attempts to authenticate to the URL with each username in the textfile. By monitoring the differences inÂ HTTPÂ response codes from the application, we can determine if the credential pair is valid or not. If the credential pair is valid, the application would respond with a 200Â HTTPÂ (OK) code. If the pair is invalid, the application will return a 401Â HTTPÂ (Unauthorised) code.

## Password Spraying  

If you are using the AttackBox, the password spraying script and usernames textfile is provided under theÂ `/root/Rooms/BreachingAD/task3/`Â directory. We can run the script using the following command:
...
- `<userfile>`Â - Textfile containing our usernames -Â _"usernames.txt"_
- `<fqdn>`Â - Fully qualified domain name associated with the organisation that we are attacking -Â _"za.tryhackme.com"_
- `password`Â - The password we want to use for our spraying attack
- Â _"Changeme123"_
- `<attackurl>`Â - The URL of the application that supports Windows Authentication -Â _"http://ntlmauth.za.tryhackme.com"_
...
Using a combination ofÂ OSINTÂ and NetNTLM password spraying, we now have our first valid credentials pairs that could be used to enumerateÂ ADÂ further!
...
![Pasted image 20250526130951.png](../../../IMAGES/Pasted%20image%2020250526130951.png)

We got our answers:

![Pasted image 20250526131002.png](../../../IMAGES/Pasted%20image%2020250526131002.png)
...
Another method ofÂ ADÂ authentication that applications can use is Lightweight Directory Access Protocol (LDAP) authentication. LDAP authentication is similar toÂ NTLMÂ authentication. However, with LDAP authentication, the application directly verifies the user's credentials. The application has a pair ofÂ ADÂ credentials that it can use first to query LDAP and then verify theÂ ADÂ user's credentials.

LDAP authentication is a popular mechanism with third-party (non-Microsoft) applications that integrate withÂ AD. These include applications and systems such as:
...
If any of these applications or services are exposed on the internet, the same type of attacks as those leveraged againstÂ NTLMÂ authenticated systems can be used. However, since a service using LDAP authentication requires a set ofÂ ADÂ credentials, it opens up additional attack avenues. In essence, we can attempt to recover theÂ ADÂ credentials used by the service to gain authenticated access toÂ AD. The process of authentication through LDAP is shown below:
...
If you could gain a foothold on the correct host, such as a Gitlab server, it might be as simple as reading the configuration files to recover theseÂ ADÂ credentials. These credentials are often stored in plain text in configuration files since the security model relies on keeping the location and storage configuration file secure rather than its contents. Configuration files are covered in more depth in Task 7.
...
LDAP Pass-back attacks can be performed when we gain access to a device's configuration where the LDAP parameters are specified. This can be, for example, the web interface of a network printer. Usually, the credentials for these interfaces are kept to the default ones, such asÂ `admin:admin`Â orÂ `admin:password`. Here, we won't be able to directly extract the LDAP credentials since the password is usually hidden. However, we can alter the LDAP configuration, such as the IP or hostname of the LDAP server. In an LDAP Pass-back attack, we can modify this IP to our IP and then test the LDAP configuration, which will force the device to attempt LDAP authentication to our rogue device. We can intercept this authentication attempt to recover the LDAP credentials.

## Performing an LDAP Pass-back  

There is a network printer in this network where the administration website does not even require credentials. Navigate toÂ [http://printer.za.tryhackme.com/settings.aspx](http://printer.za.tryhackme.com/settings.aspx)Â to find the settings page of the printer:
...
Note that if you use the AttackBox, the you should first disable slapd usingÂ `service slapd stop`. Then, we can alter the Server input box on the web application to point to our IP and press Test Settings.

**Your IP will be your VPN IP and will either be a 10.50.x.x IP or 10.51.x.x IP.Â  You can use**Â `ip a`Â **to list all interfaces. Please make sure to use this as your IP, otherwise you will not receive a connection back. Please also make note of the interface for this IP, since you will need it later in the task.**
...
For the DNS domain name, you want to provide our target domain, which isÂ `za.tryhackme.com`:
...
- **olcSaslSecProps:**Â Specifies the SASL security properties
- **noanonymous:**Â Disables mechanisms that support anonymous login
- **minssf:**Â Specifies the minimum acceptable security strength with 0, meaning no protection.
...
Also, note thatÂ `password11`Â is an example. The password for your service will be different. You may have to press the "Test Settings" button a couple of times before the TCPdump will return data since we are performing the attack over aÂ VPNÂ connection.  

Now we have another set of validÂ ADÂ credentials! By using an LDAP pass-back attack and downgrading the supported authentication mechanism, we could intercept the credentials in cleartext.
...
![Pasted image 20250526134159.png](../../../IMAGES/Pasted%20image%2020250526134159.png)
...
These services have to use built-in authentication methods to verify the identity of incoming connections. In Task 2, we exploredÂ NTLMÂ Authentication used on a web application. In this task, we will dive a bit deeper to look at how this authentication looks from the network's perspective. However, for this task, we will focus on NetNTLM authentication used byÂ SMB.

## Server Message Block

The Server Message Block (SMB) protocol allows clients (like workstations) to communicate with a server (like a file share). In networks that use MicrosoftÂ AD,Â SMBÂ governs everything from inter-network file-sharing to remote administration. Even the "out of paper" alert your computer receives when you try to print a document is the work of theÂ SMBÂ protocol.

However, the security of earlier versions of theÂ SMBÂ protocol was deemed insufficient. Several vulnerabilities and exploits were discovered that could be leveraged to recover credentials or even gain code execution on devices. Although some of these vulnerabilities were resolved in newer versions of the protocol, often organizations do not enforce the use of more recent versions since legacy systems do not support them. We will be looking at two different exploits for NetNTLM authentication withÂ SMB:

- Since theÂ NTLMÂ Challenges can be intercepted, we can use offline cracking techniques to recover the password associated with theÂ NTLMÂ Challenge. However, this cracking process is significantly slower than crackingÂ NTLMÂ hashes directly.
- We can use our rogue device to stage a man in the middle attack, relaying theÂ SMBÂ authentication between the client and server, which will provide us with an active authenticated session and access to the target server.

## LLMNR, NBT-NS, and WPAD  

In this task, we will take a bit of a look at the authentication that occurs during the use ofÂ SMB. We will use Responder to attempt to intercept the NetNTLM challenge to crack it. There are usually a lot of these challenges flying around on the network. Some security solutions even perform a sweep of entire IP ranges to recover information from hosts. Sometimes due to staleÂ DNSÂ records, these authentication challenges can end up hitting your rogue device instead of the intended host.

Responder allows us to perform Man-in-the-Middle attacks by poisoning the responses during NetNTLM authentication, tricking the client into talking to you instead of the actual server they wanted to connect to. On a real LAN, Responder will attempt to poison anyÂ  Link-Local Multicast Name Resolution (LLMNR),Â Â NetBIOSÂ Name Service (NBT-NS), and WebÂ ProxyÂ Auto-Discovery (WPAD) requests that are detected. On large Windows networks, these protocols allow hosts to perform their own localÂ DNSÂ resolution for all hosts on the same local network. Rather than overburdening network resources such as theÂ DNSÂ servers, hosts can first attempt to determine if the host they are looking for is on the same local network by sending outÂ LLMNRÂ requests and seeing if any hosts respond. The NBT-NS is the precursor protocol toÂ LLMNR, and WPAD requests are made to try and find aÂ proxyÂ for futureÂ HTTP(s) connections.

Since these protocols rely on requests broadcasted on the local network, our rogue device would also receive these requests. Usually, these requests would simply be dropped since they were not meant for our host. However, Responder will actively listen to the requests and send poisoned responses telling the requesting host that our IP is associated with the requested hostname. By poisoning these requests, Responder attempts to force the client to connect to our AttackBox. In the same line, it starts to host several servers such asÂ SMB,Â HTTP,Â SQL, and others to capture these requests and force authentication.

## Intercepting NetNTLM Challenge

One thing to note is that Responder essentially tries to win the race condition by poisoning the connections to ensure that you intercept the connection. This means that Responder is usually limited to poisoning authentication challenges on the local network. Since we are connected via aÂ VPNÂ to the network, we will only be able to poison authentication challenges that occur on thisÂ VPNÂ network. For this reason, we have simulated an authentication request that can be poisoned that runs every 30 minutes. This means that you may have to wait a bit before you can intercept the NetNTLM challenge and response.
...
Responder has already been installed on the AttackBox. However, if you are not using the AttackBox, you can download and install it from this repo:Â Â [https://github.com/lgandx/Responder](https://github.com/lgandx/Responder). We will set Responder to run on the interface connected to theÂ VPN:

`sudo responder -I breachad`  

If you are using the AttackBox not all of the Responder services will be able to start since other services are already using those ports. However, this will not impact this task. Responder will now listen for anyÂ LLMNR, NBT-NS, or WPAD requests that are coming in. We would leave Responder to run for a bit on a real LAN. However, in our case, we have to simulate this poisoning by having one of the servers attempt to authenticate to machines on theÂ VPN. Leave Responder running for a bit (average 10 minutes, get some fresh air!), and you should receive an SMBv2 connection which Responder can use to entice and extract an NTLMv2-SSP response. It will look something like this:
...
If we were using our rogue device, we would probably run Responder for quite some time, capturing several responses. Once we have a couple, we can start to perform some offline cracking of the responses in the hopes of recovering their associatedÂ NTLMÂ passwords. If the accounts have weak passwords configured, we have a good chance of successfully cracking them. Copy the NTLMv2-SSP Hash to a textfile. We will then use the password list provided in the downloadable files for this task and Hashcat in an attempt to crack the hash using the following command:

`hashcat -m 5600 <hash file> <password file> --force`

The password file has been provided for you on the AttackBox in theÂ `/root/Rooms/BreachingAD/task5/`Â directory or as a downloadable task file. We use hashtype 5600, which corresponds with NTLMv2-SSP for hashcat. If you use your own machine, you will have to installÂ [Hashcat](https://hashcat.net/hashcat/)Â first.  

Any hashes that we can crack will now provide us withÂ ADÂ credentials for our breach!  
...
- SMBÂ Signing should either be disabled or enabled but not enforced. When we perform a relay, we make minor changes to the request to pass it along. IfÂ SMBÂ signing is enabled, we won't be able to forge the message signature, meaning the server would reject it.
...
- Since we technically don't yet have anÂ ADÂ foothold, some guesswork is involved into what accounts will have permissions on which hosts. If we had already breachedÂ AD, we could perform some initial enumeration first, which is usually the case.

This is why blind relays are not usually popular. Ideally, you would first breachÂ ADÂ using another method and then perform enumeration to determine the privileges associated with the account you have compromised. From here, you can usually perform lateral movement for privilege escalation across the domain. However, it is still good to fundamentally under how a relay attack works, as shown in the diagram below:
...
If you want to try this type of attack in action, head over to theÂ [Holo Network](https://tryhackme.com/jr/hololive). We will also come back to this one in futureÂ ADÂ Rooms.
...
![Pasted image 20250526142956.png](../../../IMAGES/Pasted%20image%2020250526142956.png)
...
```
![Pasted image 20250526143244.png](../../../IMAGES/Pasted%20image%2020250526143244.png)
...
![Pasted image 20250526143255.png](../../../IMAGES/Pasted%20image%2020250526143255.png)
...
Large organisations use PXE boot to allow new devices that are connected to the network to load and install theÂ OSÂ directly over a network connection. MDT can be used to create, manage, and host PXE boot images. PXE boot is usually integrated withÂ DHCP, which means that ifÂ DHCPÂ assigns an IP lease, the host is allowed to request the PXE boot image and start the networkÂ OSÂ installation process. The communication flow is shown in the diagram below:
...
- Inject a privilege escalation vector, such as a Local Administrator account, to gain Administrative access to theÂ OSÂ once the PXE boot has been completed.
- Perform password scraping attacks to recoverÂ ADÂ credentials used during the install.

In this task, we will focus on the latter. We will attempt to recover the deployment service account associated with the MDT service during installation for this password scraping attack. Furthermore, there is also the possibility of retrieving otherÂ ADÂ accounts used for the unattended installation of applications and services.

### PXE Boot Image Retrieval  

SinceÂ DHCPÂ is a bit finicky, we will bypass the initial steps of this attack. We will skip the part where we attempt to request an IP and the PXE boot preconfigure details fromÂ DHCP. We will perform theÂ restÂ of the attack from this step in the process manually.

The first piece of information regarding the PXE Boot preconfigure you would have received viaÂ DHCPÂ is the IP of the MDT server. In our case, you can recover that information from the TryHackMe network diagram.  

The second piece of information you would have received was the names of the BCD files. These files store the information relevant to PXE Boots for the different types of architecture. To retrieve this information, you will need to connect to this website:Â [http://pxeboot.za.tryhackme.com](http://pxeboot.za.tryhackme.com/). It will list various BCD files:
...
Usually, you would use TFTP to request each of these BCD files and enumerate the configuration for all of them. However, in the interest of time, we will focus on the BCD file of theÂ **x64**Â architecture. Copy and store the full name of this file. For theÂ restÂ of this exercise, we will be using this name placeholderÂ `x64{7B...B3}.bcd`Â since the files and their names are regenerated by MDT every day. Each time you see this placeholder, remember to replace it with your specific BCD filename.Â **Note as well that if the network has just started, these file names will only update after 10 minutes of the network being active.**  
...
and the password ofÂ `Password1@`.

To ensure that all users of the network can useÂ SSH, start by creating a folder with your username and copying the powerpxe repo into this folder:
...
The first step we need to perform is using TFTP and downloading our BCD file to read the configuration of the MDT server. TFTP is a bit trickier thanÂ FTPÂ since we can't list files. Instead, we send a file request, and the server will connect back to us viaÂ UDPÂ to transfer the file. Hence, we need to be accurate when specifying files and file paths. The BCD files are always located in the /Tmp/ directory on the MDT server. We can initiate the TFTP transfer using the following command in ourÂ SSHÂ session:
...
You will have to lookup THMMDT IP withÂ `nslookup thmmdt.za.tryhackme.com`. With the BCD file now recovered, we will be usingÂ [powerpxe](https://github.com/wavestone-cdt/powerpxe)Â to read its contents. Powerpxe is aÂ PowerShellÂ script that automatically performs this type of attack but usually with varying results, so it is better to perform a manual approach. We will use theÂ Get-WimFile function of powerpxe to recover the locations of the PXE Boot images from the BCD file:
...
Now that we have recovered the PXE Boot image, we can exfiltrate stored credentials. It should be noted that there are various attacks that we could stage. We could inject a local administrator user, so we have admin access as soon as the image boots, we could install the image to have a domain-joined machine. If you are interested in learning more about these attacks, you can read thisÂ [article](https://www.riskinsight-wavestone.com/en/2020/01/taking-over-windows-workstations-pxe-laps/). This exercise will focus on a simple attack of just attempting to exfiltrate credentials.
...
As you can see, powerpxe was able to recover theÂ ADÂ credentials. We now have another set ofÂ ADÂ credentials that we can use!
...
![Pasted image 20250526145243.png](../../../IMAGES/Pasted%20image%2020250526145243.png)
...
![Pasted image 20250526145737.png](../../../IMAGES/Pasted%20image%2020250526145737.png)

![Pasted image 20250526145757.png](../../../IMAGES/Pasted%20image%2020250526145757.png)
...
![Pasted image 20250526145924.png](../../../IMAGES/Pasted%20image%2020250526145924.png)
...
![Pasted image 20250526150308.png](../../../IMAGES/Pasted%20image%2020250526150308.png)
...
![Pasted image 20250526150415.png](../../../IMAGES/Pasted%20image%2020250526150415.png)

There we go, we got the credentials.

![Pasted image 20250526150453.png](../../../IMAGES/Pasted%20image%2020250526150453.png)
...
Several enumeration scripts, such asÂ [Seatbelt](https://github.com/GhostPack/Seatbelt), can be used to automate this process.
...
McAfee embeds the credentials used during installation to connect back to the orchestrator in a file called ma.db. This database file can be retrieved and read with local access to the host to recover the associatedÂ ADÂ service account. We will be using theÂ SSHÂ access on THMJMP1 again for this exercise.
...
We now once again have a set ofÂ ADÂ credentials that we can use for further enumeration! This is just one example of recovering credentials from configuration files. If you are ever able to gain a foothold on a host, make sure to follow a detailed and refined methodology to ensure that you recover all loot from the host, including credentials and other sensitive information that can be stored in configuration files.
...
![Pasted image 20250526152524.png](../../../IMAGES/Pasted%20image%2020250526152524.png)
...
![Pasted image 20250526152739.png](../../../IMAGES/Pasted%20image%2020250526152739.png)
...
![Pasted image 20250526153507.png](../../../IMAGES/Pasted%20image%2020250526153507.png)
...
![Pasted image 20250526153536.png](../../../IMAGES/Pasted%20image%2020250526153536.png)

# Conclusion

---

A significant amount of attack avenues can be followed to breachÂ AD. We covered some of those commonly seen being used during a red team exercise in this network. Due to the sheer size of the attack surface, new avenues to recover that first set ofÂ ADÂ credentials are constantly being discovered. Building a proper enumeration methodology and continuously updating it will be required to find that initial pair of credentials.
...
- Limit the exposure ofÂ ADÂ services and applications online - Not all applications must be accessible from the internet, especially those that supportÂ NTLMÂ and LDAP authentication. Instead, these applications should be placed in an intranet that can be accessed through aÂ VPN. TheÂ VPNÂ can then support multi-factor authentication for added security.
...
- EnforceÂ SMBÂ Signing - By enforcingÂ SMBÂ signing,Â SMBÂ relay attacks are not possible.
- Follow the principle of least privileges - In most cases, an attacker will be able to recover a set ofÂ ADÂ credentials. By following the principle of least privilege, especially for credentials used for services, the risk associated with these credentials being compromised can be significantly reduced.

Now that we have breachedÂ AD, the next step is to perform enumeration ofÂ ADÂ to gain a better understanding of the domain structure and identify potential misconfigurations that can be exploited. This will be covered in the next room. Remember to clear theÂ DNSÂ configuration!
