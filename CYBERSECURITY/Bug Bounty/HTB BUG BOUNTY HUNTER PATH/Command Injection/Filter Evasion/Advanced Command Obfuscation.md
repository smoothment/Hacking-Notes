...
Â Â Advanced Command Obfuscation
...
1. **Base64 Encoded Command**  
    The original commandÂ `find /usr/share/ | grep root | grep mysql | tail -n 1`Â is encoded to:  
    `ZmluZCAvdXNyL3NoYXJlLyB8IGdyZXAgcm9vdCB8IGdyZXAgbXlzcWwgfCB0YWlsIC1uIDEK`.
    
2. **Decode and Execute**
    
    - `base64${IFS}-d`: Decodes the Base64 string (usingÂ `${IFS}`Â for space).
        
    - `bash<<<`: Passes the decoded command toÂ `bash`Â for execution.
