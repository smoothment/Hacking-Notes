...
So far, we have not been using any filtering to ourÂ `ffuf`, and the results are automatically filtered by default by their HTTP code, which filters out codeÂ `404 NOT FOUND`, and keeps the rest. However, as we saw in our previous run ofÂ `ffuf`, we can get many responses with codeÂ `200`. So, in this case, we will have to filter the results based on another factor, which we will learn in this section.



## Filtering

`Ffuf`Â provides the option to match or filter out a specific HTTP code, response size, or amount of words. We can see that withÂ `ffuf -h`:

Â Â Filtering Results
...
In this case, we cannot use matching, as we don't know what the response size from other VHosts would be. We know the response size of the incorrect results, which, as seen from the test above, isÂ `900`, and we can filter it out withÂ `-fs 900`. Now, let's repeat the same previous command, add the above flag, and see what we get:
...
We see that we can access the page, but we get an empty page, unlike what we got withÂ `academy.htb`, therefore confirming this is indeed a different VHost. We can even visitÂ `https://admin.academy.htb:PORT/blog/index.php`, and we will see that we would get aÂ `404 PAGE NOT FOUND`, confirming that we are now indeed on a different VHost.

Try running a recursive scan onÂ `admin.academy.htb`, and see what pages you can identify.
