...
But, here's where another form of XSS comes in handy, we'll be using `OOB XSS`, OOB xss or Out-Of-Band XSS is a technique where a hostile script doesnâ€™t run visibly in the immediate HTTP response. Instead, it leverages a side channel, like a DNS query, an HTTP callback, or a WebSocket message, to quietly confirm execution or siphon off data.
...
First of all, we are part of the `adm` group, ADM group stands for `admin log reader`. Itâ€™s a Linux group that gives user access to log files stored in /var/log , like auth.log for example.
